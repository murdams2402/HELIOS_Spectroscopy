...
background = pd.read_table("background_noise_raw_data.txt", sep=" ", names=["wavelegnth", "intensiy"], skiprows=2)
...
data = pd.DataFrame(columns=["pressure", "moment", "wavelegnth", "intensiy"])
...
while pressure > 1e-8:
    pressure = input("Enter actual pressure value [mbar]: p = ")
    now = datetime.now()
    dt_string = now.strftime("%d:%m:%y_%H-%M-%S")

    # Getting data: measuring plasma spectrum 
    get_snapshot(integration_time=integration_time, name="doppler_broadening_data_" + dt_string, start=start, end=end)
    
    # Importing data 
    brut = pd.read_table(f"doppler_broadening_data\_pressure={pressure}mbar\_time=" + dt_string, 
                         sep=" ", 
                         names=["wavelegnth", "intensiy"], 
                         skiprows=2)
    raw = [(pressure, now, lambda_, I) for lambda_, I in zip(brut['wavelegnth'], brut["intensiy"])]
    data_ = pd.DataFrame(raw, columns=["pressure", "moment", "wavelegnth", "intensiy"])
    data_["intensiy"] = data_["intensiy"] - background["intensiy"][start-1:end+1]
